...
export function CombosManager({ initialRestaurants }: { initialRestaurants: Restaurant[] }) {
  ...
  function upsertComboInState(combo: Combo) {
    setRestaurants((current) =>
      current.map((entry) =>
        entry.id !== activeRestaurantId
          ? entry
          : {
              ...entry,
              combos: entry.combos.some((existing) => existing.id === combo.id)
                ? entry.combos.map((existing) => (existing.id === combo.id ? combo : existing))
                : [combo, ...entry.combos]
            }
      )
    );
  ...
  async function save() {
    ...
    try {
      ...
      upsertComboInState(combo as Combo);
  ...
  async function toggleActive(combo: Combo) {
    try {
      ...
      upsertComboInState(updated as Combo);
    } catch (error) {
  ...
  async function removeCombo(combo: Combo) {
    ...
    try {
      ...
      setRestaurants((current) =>
        current.map((entry) => (entry.id !== activeRestaurantId ? entry : { ...entry, combos: entry.combos.filter((existing) => existing.id !== combo.id) }))
      );
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
        ...
        <SectionCard
          ...
          description={restaurant ? `Pick items from ${restaurant.name}, then set your bundle price.` : undefined}
      ...
      {/* Existing combos */}
      <SectionCard title="Combos on this restaurant" description={combos.length ? `${combos.length} combo${combos.length === 1 ? "" : "s"} — customers see these first.` : undefined}>
        {combos.length ? (
